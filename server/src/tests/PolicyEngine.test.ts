...
import type { Policy, MockResponses, PolicyVerdict, Logger } from '../types';
 ...
 */
async function testAllRulesPass(): Promise<void> {
  console.log('\n========================================');
  ...
  const verdict: PolicyVerdict = await engine.evaluate(content, { policy: testPolicy });
 ...
 */
async function testCriticalRuleFails(): Promise<void> {
  console.log('\n========================================');
 ...
 */
async function testWeightedPolicyPasses(): Promise<void> {
  console.log('\n========================================');
  ...
  const verdict: PolicyVerdict = await engine.evaluate(content, { policy: weightedPolicy });

  assert(verdict.final_verdict === 'ALLOW', 'Final verdict should be ALLOW');
  assert(verdict.passed === true, 'Passed should be true');
  assert(verdict.summary?.strategy === 'weighted_threshold', 'Strategy should be weighted_threshold');
  assert((verdict.summary?.score ?? 0) >= 0.6, 'Score should be >= threshold');
  assert(verdict.rule_results.filter(r => r.verdict === 'FAIL').length === 1, 'Should have 1 failed rule');
 ...
 */
async function testLLMTimeout(): Promise<void> {
  console.log('\n========================================');
 ...
 */
async function testUncertainVerdict(): Promise<void> {
  console.log('\n========================================');
  ...
  assert((uncertainRule?.confidence ?? 1) < 0.5, 'Uncertain rule should have low confidence');
 ...
 */
async function testAnyStrategy(): Promise<void> {
  console.log('\n========================================');
  ...
  const verdict: PolicyVerdict = await engine.evaluate(content, { policy: anyPolicy });
 ...
 */
async function testPolicyValidation(): Promise<void> {
  console.log('\n========================================');
  ...
  const invalidPolicy2 = { 
    name: 'test', 
    rules: [{ id: 'r1', judge_prompt: 'test', on_fail: 'block' as const }],
    evaluation_strategy: 'invalid_strategy' as unknown as Policy['evaluation_strategy']
  };
